...
public class NPC extends Character {
    public void reciveItem(Item item) {
        switch (item) {
            case QuestItem questItem -> questItem.setRedeemed(true);
            case Consumable consumable -> this.useItem(consumable);
            case Equipment equipment -> this.equipItem(equipment);
            default -> Player.addItem(item);
        }
        addItem(item);
